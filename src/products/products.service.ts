...
export class ProductsService {
  ...
  async create(dto: CreateProductDto): Promise<ApiResponse<Product>> {
    ...
    const bySlug = await this.repo.findOne({ where: { slug } });
    if (bySlug) throw new BadRequestException("Product with this slug already exists");
  ...
  async update(dto: UpdateProductDto): Promise<ApiResponse<Product>> {
    ...
    const slug = dto.slug.trim();
    const conflictSlug = await this.repo.findOne({ where: { slug } });
    if (conflictSlug && conflictSlug.id !== dto.id) {
  ...
  async getAll(paginationDto: PaginationDto): Promise<PaginatedApiResponse<Product>> {
    const { page = 1, limit = 10 } = paginationDto;
    const skip = (page - 1) * limit;

    const [items, total] = await this.repo.findAndCount({
      skip, take: limit, order: { created_at: "DESC" },
      // relations: ["category"], // uncomment if you want category object included
    });
